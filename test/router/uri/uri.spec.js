/**
 * @file uri spec
 * @author treelite(c.xinle@gmail.com)
 */

define(function (require) {

    var uri = require('router/uri');

    describe('router/uri', function () {

        describe('#toString()', function () {

            it('should return the right string', function () {
                var str = 'www.baidu.com';
                expect(uri(str).toString()).to.deep.equal(str);

                str = 'http://www.baidu.com/search?wd=hello';
                expect(uri(str).toString()).to.deep.equal(str);

                str = 'www.baidu.com/?search=wd';
                expect(uri(str).toString()).to.deep.equal(str);
            });

        });

        describe('#get()', function () {
            it('should return component data', function () {
                var url = uri('www.baidu.com/search');

                expect(url.get('scheme')).to.equal('');
                expect(url.get('username')).to.equal('');
                expect(url.get('password')).to.equal('');
                expect(url.get('host')).to.deep.equal('www.baidu.com');
                expect(url.get('port')).to.equal('');
                expect(url.get('path')).to.deep.equal('/search');
                expect(url.get('query')).to.deep.equal({});
                expect(url.get('fragment')).to.equal('');
            });

            it('should return query data', function () {
                var url = uri('www.baidu.com/search?wd=hello&page=10');

                expect(url.get('query', 'wd')).to.deep.equal('hello');
                expect(url.get('query', 'page')).to.deep.equal('10');
            });
        });

        describe('#set()', function () {
            it('without argument should pass', function () {
                var url = uri('www.baidu.com');
                var str = 'www.google.com/?query=wd';

                url.set(str);

                expect(url.toString()).to.deep.equal(str);
                expect(url.path.toString()).to.deep.equal('/');
                expect(url.query.toString()).to.deep.equal('?query=wd');
            });
        });

        describe('#equal()', function () {
            it('width string should return true when they are the same', function () {
                expect(uri('www.baidu.com?wd=hello#no').equal('www.baidu.com?wd=hello#no')).to.be.ok;
            });

            it('with string should return fase when they are not the same', function () {
                expect(uri('www.baidu.com?wd=hello#no').equal('www.baidu.com')).to.not.be.ok;
            });

            it('width URI object should return true when they are the same', function () {
                var aURI = uri('www.baidu.com?wd=hello#no');
                var bURI = uri('www.baidu.com?wd=hello#no');
                expect(aURI.equal(bURI)).to.be.ok;
            });

            it('with URI object should return fase when they are not the same', function () {
                var aURI = uri('www.baidu.com?wd=hello#no');
                var bURI = uri('www.baidu.com');
                expect(aURI.equal(bURI)).to.not.be.ok;
            });

            it('should not case sensitive about host', function () {
                expect(uri('www.BAIDU.COM').equal('www.baidu.com')).to.be.ok;
            });

            it('should not case sensitive about scheme', function () {
                expect(uri('HTTps://www.baidu.com').equal('https://www.baidu.com')).to.be.ok;
            });

            it('should case default port', function () {
                expect(uri('https://www.baidu.com').equal('https://www.baidu.com:443')).to.be.ok;
            });

            it('should normalize path', function () {
                expect(uri('https://www.baidu.com').equal('https://www.baidu.com/')).to.be.ok;
                expect(uri('https://www.baidu.com?xxx').equal('https://www.baidu.com/?xxx')).to.be.ok;
            });
        });

    });

});
